...
class TTSWindow(ThemeEngine):
    def __init__(self, master):
        ...
        self.add_prod_win.title("Emploi du temps Salle {}".format(self.gsalle))
    ...
    def  voir_timetable_db(self):
        self.cur.execute("SELECT * FROM timetable_tp  WHERE local =?", (self.gsalle,))
        ...
        if len(ll)>0:
            ...
            for row in ll:
                ...
                if (row[1]%3==0):
                        self.cr_vars[z].set(row[-1])
                        self.cr_vars[z].set('{0}-{1}-{2}-{3}'.format(row[3],row[4],row[5],row[6]))
        else: 
            messagebox.showinfo("Validation info", "Emplois de la salle {} n'est pas existe".format(self.gsalle))
            return
    ...
    def timetabling(self):
        ...
        self.left_course = []
        self.tasks = []
        self.cur.execute("SELECT module FROM timetable_tp  WHERE local =?", (self.gsalle,))
        mod=self.cur.fetchall()
        if len(list(mod))!=0:
            for i in mod:
                self.tasks.append(i[0])
        self.cur.execute("SELECT filiere, semestre, section, groupe FROM timetable_tp  WHERE local =?", (self.gsalle,))
        ...
        for i in range(1,19):
            for j in range(1,6):                
                self.left_prof = []
                self.left_prof.append('')

                self.left_salle = []
                self.left_salle.append('')
    ...
    def makeprint_emploi(self):
           if messagebox.askyesno("Alerte!","Imprimer cette emploi ?")== True:
                ...
                canv = canvas.Canvas("emploi/Salle {}.pdf".format(self.gsalle), pagesize=landscape(A4))
                canv.setLineWidth(.3)
                canv.setFont('Helvetica', 13)
                canv.drawImage('./images/FPO.png', 50, 530, width=290, height=60)
                canv.line(50, 515,800 ,515) 
                ...
                canv.showPage()
                canv.save()
                subprocess.Popen(["emploi/Salle {}.pdf".format(self.gsalle)],shell=True)  
